        ...
        try:
            if message.topic == "fingerprint":
                ...
                if current_beacon == None:
                    ...
                    current_beacon = Beacon(mac_address=advertising_device_mac,
                                            beacon_name="beacon" + str(registered_beacons_count + 1))

                Advertisement(advertised_device=current_beacon, rssi=rssi, advertised_channel=channel, date=now,
                              location="unknown", scan_number=self.scan_counter)
                ...
                if message.payload.decode('utf-8') == "T":
                    ...
                    if len(unlabeled) > 200:
                        for i in unlabeled[0:len(unlabeled) - 200]:
                            #    print(len(unlabeled))
                            i.location = "lost"
                    unlabeled = select(f for f in Advertisement if f.location == "unknown")[:]
                    if len(unlabeled) != 0:
                        fingerprint = Fingerprint(scan_number=self.scan_counter, date=unlabeled[0].date,
                                                  location="unknown")
                        for f in unlabeled:
                            setattr(fingerprint, getattr(f, 'advertised_device').beacon_name, f.rssi)
                            setattr(fingerprint, self.attributes[
                                self.attributes.index(getattr(f, 'advertised_device').beacon_name) + 1],
                                    f.advertised_channel)
